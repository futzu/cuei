...
func (upid *Upid) Decoder(gob *gob.Gob, upidType uint8,  upidlen uint8) {
...
// Decode for AirId
func (upid *Upid) AirID(gob *gob.Gob, upidlen uint8) {
	upid.Value = gob.Hex(uint(upidlen << 3))
}


// Decode for Isan Upid
func (upid *Upid) ISAN(gob *gob.Gob, upidlen uint8) {
	upid.Value = gob.Ascii(uint(upidlen << 3))
}

// Decode for URI Upid
func (upid *Upid) URI(gob *gob.Gob, upidlen uint8) {
	upid.Value = gob.Ascii(uint(upidlen) << 3)
}

// Decode for ATSC Upid
func (upid *Upid) ATSC(gob *gob.Gob, upidlen uint8) {
...
func (upid *Upid) EIDR(gob *gob.Gob, upidlen uint8) {
	if upidlen == 12 {
		head := gob.UInt64(16)
		tail := gob.Hex(80)
...
// Decode for MPU Upid
func (upid *Upid) MPU(gob *gob.Gob, upidlen uint8) {
	ulb := uint(upidlen) << 3
...
// Decode for MID Upid
func (upid *Upid) MID(gob *gob.Gob, upidlen uint8) {
	var i uint8
	i = 0
	for i < upidlen {
		utype := gob.UInt8(8)
